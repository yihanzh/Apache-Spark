  ...
  def main(args: Array[String]) {
    ...
    lengths.foreachRDD((rdd, time) => {
      ...
      if (count > 0) {
        ...
        if (rdd.max() > longestTweet.get())
          longestTweet.set(rdd.max())
        // another solution
        //if (rdd.reduce((x,y) => x max y) > longestTweet.get())
            //longestTweet.set(rdd.reduce((x,y) => x max y))
        
        println("Total tweets: " + totalTweets.get() + 
            " Total characters: " + totalChars.get() + 
            " Average: " + totalChars.get() / totalTweets.get()+ 
            " Characters in the longest tweet : " + longestTweet.get())
      }
    })
